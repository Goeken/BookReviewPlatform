...
RSpec.describe Review, type: :model do
  ...
  describe 'validations' do
    ...
    let!(:book) { Book.create!(title: 'Test Book', author: 'Author Name', publication_year: 2023, isbn: '1234567890', user: user) }
    let!(:existing_review) { described_class.create!(content: 'Great book!', rating: 5, user: user, book: book) }

    it { should validate_presence_of(:content) }
    it { should validate_inclusion_of(:rating).in_range(1..5) }

    it 'validates uniqueness of user_id scoped to book_id' do
      new_review = described_class.new(content: 'Another review', rating: 4, user: user, book: book)
